...
def H_q(Q, Qzp, wavefn, lambda_alpha):
    ...
    for n in range(num_trj):
        ground_pop = np.abs(wavefn[0,n])**2
        #ex_pop = np.abs(wavefn[1, n]) ** 2
        intQ = Q[:,n] + (1-ground_pop) * Qzp[:,n]
...
def get_eigs(H):
    ...
    for i in np.arange(v1[0,:].size):
        if np.abs(v[i]) < 1e-14:
            ...
            v1[0, i] = v[i] / 2
            v2[1, i] = v[i] / 2
    de_v1=np.sqrt(np.sum(v1 * v1, axis=0))
    de_v2=np.sqrt(np.sum(v2 * v2, axis=0))
    v1 = v1 / de_v1
    v2 = v2 / de_v2
...
def run_dyn(index, ini_wavefn, Q, P, QaQb, Qzp, Pzp, QaQbzp, lambda_al):
    ...
    for n in np.arange(0, num_atom):
        qE[n, :], qcE[n, :] = quantum_energy(np.array([wavefn[2 * n], wavefn[2 * n + 1]]), Q, Qzp, lambda_al[:, n])
    ...
    for t in steps:
        ...
        QFzp = 0
        for n in np.arange(0, num_atom):
            now_wavefn = prop_Q_exact(np.array([wavefn[2*n], wavefn[2*n+1]]), Q, Qzp, lambda_al[:,n], dt)
            wavefn[2*n] = now_wavefn[0]
            wavefn[2*n+1] = now_wavefn[1]
            nowQF = Q_F(now_wavefn, lambda_al_grid[n,:,:])
            QF = QF + nowQF
            QFzp = QFzp + (1-(np.abs(now_wavefn[0])**2)) * nowQF
