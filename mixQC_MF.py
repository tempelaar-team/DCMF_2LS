...
# NOTE: in this version, P and Q stand for <P> and <Q>
...
def Q_F(wavefn, lambda_alpha_grid):
    force = 2 * wgrid * nu12 * lambda_alpha_grid * np.conjugate(wavefn[0]) * wavefn[1]
    return force

@jit(nopython=True)
def H_q(Q, lambda_alpha):
...
def get_eigs(H):
    ...
    delta_e = e1 - e2
    e = e1 + e2
    l1 = (1/2) * ( e - np.sqrt(v**2 + delta_e**2))
    l2 = (1/2) * ( e + np.sqrt(v**2 + delta_e**2))
    v1 = np.ascontiguousarray(np.zeros((2, np.shape(H)[-1])))+0.0j
    v2 = np.ascontiguousarray(np.zeros((2, np.shape(H)[-1]))) + 0.0j
    v1[0,:] = v/2
    v1[1,:] = l1 - e1
    v2[0,:] = l2 - e2
    v2[1,:] = v/2
    v1 = v1/np.sqrt(np.sum(v1 * v1, axis=0))
    v2 = v2/np.sqrt(np.sum(v2 * v2, axis=0))
    eigvals = np.ascontiguousarray(np.zeros((2,np.shape(H)[-1])))
    eigvecs = np.ascontiguousarray(np.zeros(np.shape(H)))+0.0j
    eigvals[0,:] = np.real(l1)
    eigvals[1,:] = np.real(l2)
    eigvecs[:,0,:] = v1
    eigvecs[:,1,:] = v2
    return eigvals, eigvecs

#@jit(nopython=True,fastmath=True)
def prop_Q_exact(psiA, q, lambda_alpha, dt):
    evalsA, evecsA = get_eigs(H_q(q, lambda_alpha))
    evalsA_exp = np.exp(-1.0j * evalsA * dt)
    psiA_adb = vec_db_to_adb(psiA, evecsA)
    psiA_adb[0,:] = evalsA_exp[0,:] * psiA_adb[0,:]
    psiA_adb[1,:] = evalsA_exp[1,:] * psiA_adb[1,:]
    psiA_db = vec_adb_to_db(psiA_adb, evecsA)
    return psiA_db
...
#@jit(nopython=True)
def quantum_energy(wavefn, Q, lambda_alpha):
    bra = np.conjugate(wavefn)
    q_energy = np.zeros(num_trj)
    qc_energy = np.zeros(num_trj)
    Hq=H_q(Q, lambda_alpha)
    Hq_dia=np.zeros_like(Hq)
    Hq_notdia =np.zeros_like(Hq)
    Hq_dia[0,0,:], Hq_dia[1,1,:] = Hq[0,0,:], Hq[1,1,:]
    Hq_notdia[0,1,:], Hq_notdia[1,0,:] = Hq[0,1,:], Hq[1,0,:]
    for n in range(num_trj):
        q_energy[n] = np.real(bra[:,n]@Hq_dia[:,:,n]@wavefn[:,n][:,None])
        qc_energy[n] = np.real(bra[:,n]@Hq_notdia[:,:,n]@wavefn[:,n][:,None])
...
def init_classical_parallel(wavefn, lambda_al, num_trj, proc):
    ...
    for n in range(num_trj):
        Q[:, n], P[:, n] = Wigner(wavefn, lambda_al)
    ...
    return Qsplit, Psplit, QaQbsplit
...
def initial_wavefn(ini_wavefn, num_trj, proc):
    wavefn = np.zeros([2 * num_atom, num_trj], dtype='complex64')
    for n in range(num_atom):
...
# @jit(nopython=True)
@ray.remote
def run_dyn(index, ini_wavefn, Q, P, QaQb, lambda_al):
    ...
    for n in range(num_atom):
        qE[n, :], qcE[n, :] = quantum_energy(np.array([wavefn[2 * n], wavefn[2 * n + 1]]), Q, lambda_al[:, n])
    ...
    sys_ene = np.concatenate([[class_energy(Q, P)], qE, qcE])
    ...
    for t in steps:
        QF = 0
        for n in range(num_atom):
            now_wavefn = prop_Q_exact(np.array([wavefn[2*n], wavefn[2*n+1]]), Q, lambda_al[:,n], dt)
            wavefn[2*n] = now_wavefn[0]
            wavefn[2*n+1] = now_wavefn[1]
            QF = QF + Q_F(now_wavefn, lambda_al_grid[n,:,:])
    ...
    return wavefnsquare_save, QaQb_save, np.transpose(Psquare_save), sys_energy_save, Pend, Qend, wavefnend
...
def parallel_run_ray(total_trj, proc):
    ...
    for run in range(0, int(trials/proc)):
        ...
        if 'status' in globals() and status=='RESTART':
            ...
            q, p, QaQb = init_classical_parallel(ini_wavefn, lambda_al, total_trj, trials)

        results = [run_dyn.remote(run * proc + i, wavefn[i], q[i], p[i], QaQb[i], lambda_al) for i in range(proc)]
        for r in results:
            simRhoA, simQ1Q2, simP2, simE, Pend, Qend, wavefnend = ray.get(r)
    ...
    return simRhoAdat, simP2dat, simQ1Q2dat, simEdat, Penddat, Qenddat, wavefnenddat
...
def runCalc():
    ...
    resRhoA, resP2, resQ1Q2, resE, Pend, Qend, wavefnend = parallel_run_ray(total_trj, proc)
